...
test('it should return a list of all canonical routes powered by the CMS', () => {
  return superagent.get(apiUrl('/v1/list-routes')).then(res => {
    ...
    expect(paths).toContain('/funding/programmes/national-lottery-awards-for-all-england');
    expect(paths).toContain('/funding/programmes/building-better-opportunities');
  });
});
